...
def report_detail_sheet(result, excel):
    ...
    title_style = excel.add_format(
        {'bold': True, 'align': 'center', 'font_size': 16, 'valign': 'vcenter', 'fg_color': '#f05b72'})
    detail_sheet.merge_range('A1:J2', 'APP检查报告 - ' + name, title_style)

    # app infomation show
    header_style = excel.add_format({'bold': True, 'align': 'center', 'fg_color': '#f05b72', 'border': 1})
    ...
    # new row
    detail_sheet.write(3, 0, 'app名字', header_style)
    detail_sheet.write(3, 1, name, text_style)

    detail_sheet.write(3, 2, 'app版本', header_style)
    ...
    # new row
    detail_sheet.write(4, 0, 'Architectures', header_style)
    detail_sheet.merge_range('B5:D5', ' / '.join(arcs), text_style)
    ...
    sub_title_style = excel.add_format(
        {'bold': True, 'align': 'center', 'font_size': 14, 'valign': 'vcenter', 'fg_color': '#f05b72', 'border': 1})

    base_row = 8
    table_header_style = excel.add_format({'bold': True, 'align': 'center', 'fg_color': '#a3cf62'})
    ...
    if (len(warnings) > 0):
        for temp in warnings:
            ...
            detail_sheet.merge_range(base_row, 1, base_row, 9, temp['label'] + ': ' + temp['description'],
                                     text_left_style)
    else:
        base_row = base_row + 1
    ...
    if len(private_apis) > 0:
        for api in private_apis:
            ...
            cnt = cnt + 1
            detail_sheet.write(base_row, 0, cnt, text_style)
            detail_sheet.write(base_row, 1, api.get('source_framework', ''), text_style)
...
def report_outline_sheet(private_results, excel):
    ...
    # title 合并单元格
    title_style = excel.add_format(
        {'bold': True, 'align': 'center', 'font_size': 18, 'valign': 'vcenter', 'fg_color': '#f05b72'})
    ouline_sheet.merge_range('A1:L5', '支付平台SDK上线私有API审核报告（%s）' % time.strftime("%Y-%m-%d", time.localtime()),
                             title_style)

    ########header
    header_style = excel.add_format({'bold': True, 'align': 'center', 'fg_color': '#a3cf62', 'font_size':12})
    ...
    for result in private_results:
        ...
        ouline_sheet.write_url(cnt, 1, 'internal:' + result.get('sheet_name', '') + '!' + xl_rowcol_to_cell_fast(0, 0),
                               url_format, result.get('name', ''))
        ...
        ouline_sheet.write(cnt, 11, str(len(result.get('private_apis', []))) + ' / ' + str(
            len(result.get('private_frameworks', []))), text_format)  # G
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        result = {}
        result['private_apis'] = []
        result['private_apis'].append(
            {'framework': 'rsst_fr', 'header_file': '/homwefw/wef.haewf', 'class_name': 'wfwefClass',
             'api_name': 'wfewefwf'})
        result['private_apis'].append(
            {'framework': 'rsst_fr', 'header_file': '/homwefw/wef.haewf', 'class_name': 'wfwefClass',
             'api_name': 'wfewefwf'})
    ...
    # test report excel
    excel_path = utils.get_digest_str() + '.xlsx'
    print(excel_path)
